...
\begin{frame}[fragile]{Query Transition Graph}
    ...
    \begin{itemize}
        ...
        \visible<2->{
        \item{Which query templates are \alert{correlated} with each other. If correlation is sufficient high ($> \tau$ threshold), then we
...
Key query results by \alert{hash of query template} and parameter values.
\begin{itemize}
